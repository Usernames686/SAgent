...
import { STEPS, TOTAL_STEPS, type StepSection } from '@/components/vibe/detail/seven-types-steps';
...
export default function SevenTypesDetailPage() {
    ...
    <div className="h-full overflow-y-auto" style={{ background: 'linear-gradient(135deg, #020614 0%, #0a0f1e 50%, #0d1321 100%)' }}>
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold text-white mb-2">类型图鉴 — JavaScript 七种原始类型</h1>
          <p className="text-sm text-white/65">由浅入深，图文并茂，逐个掌握每种数据类型</p>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="h-1.5 bg-white/[0.05] rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-500 rounded-full transition-all duration-500"
            ...
            return (
              <button key={i} onClick={() => setCurrentStep(i)}
                className={`shrink-0 px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all ${
                  isCurrent ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/20'
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-5">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 flex items-center justify-center shrink-0">
              <span className="text-sm font-bold text-cyan-400">{currentStep + 1}</span>
            </div>
            <div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/[0.06]">
          ...
          {currentStep < TOTAL_STEPS - 1 ? (
            <button onClick={goNext}
              className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-medium text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 transition-all shadow-lg shadow-cyan-500/20">
